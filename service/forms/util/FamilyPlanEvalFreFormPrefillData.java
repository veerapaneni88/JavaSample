...
@Component
public class FamilyPlanEvalFreFormPrefillData extends DocumentServiceUtil {
	...
	public PreFillDataServiceDto returnPrefillData(Object parentDtoobj) {
		...
		for (FamilyPlanPartcpntDto familyPlanPartcpntDto : familyPlanDtlEvalRes.getFamilyPlanPartcpntDto()) {
			if (familyPlanPartcpntDto.getIndPartcpntType().equalsIgnoreCase("P")) {
				...
				List<BookmarkDto> additionalInfoBookmarkList = new ArrayList<BookmarkDto>();
				BookmarkDto addBookmarkAddInfo = createBookmark(BookmarkConstants.ADDITIONAL_INFO_2,
						formatTextValue(familyPlanPartcpntDto.getTxtAdtnlInfo()));
				additionalInfoBookmarkList.add(addBookmarkAddInfo);
		...
		for (FamilyPlanPartcpntDto familyPlanPartcpntDto : familyPlanDtlEvalRes.getFamilyPlanPartcpntDto()) {
			if (familyPlanPartcpntDto.getIndPartcpntType().equalsIgnoreCase("P")) {
				...
				for (FamilyPlanNeedsDto familyPlanNeedsDto : familyPlanPartcpntDto.getFamilyPlanNeedsDtoList()) {
					...
					List<BookmarkDto> ParentStrQuesBookmarkList = new ArrayList<BookmarkDto>();
					if (FormConstants.Y.equalsIgnoreCase(familyPlanNeedsDto.getIndPriorNeeds())) {
						...
						BookmarkDto addBookmarkCurrAssmtNeeds = createBookmark(BookmarkConstants.CURR_ASSMT_NEED,
								formatTextValue(familyPlanNeedsDto.getTxtCurrAssmntNeed()));
						BookmarkDto addBookmarkNewNeeds = createBookmark(BookmarkConstants.ASSMT_NEED,
								formatTextValue(familyPlanNeedsDto.getTxtNeedDescription()));
						BookmarkDto addBookmarkEvalProgress = createBookmark(BookmarkConstants.PROGRESS_EVAL,
								formatTextValue(familyPlanNeedsDto.getTxtEvalProgress()));

						ParentStrQuesBookmarkList.add(addBookmarkCurrAssmtNeeds);
						ParentStrQuesBookmarkList.add(addBookmarkNeedDomain);
						ParentStrQuesBookmarkList.add(addBookmarkRsnAddtn);
						ParentStrQuesBookmarkList.add(addBookmarkEvalProgress);
						ParentStrQuesBookmarkList.add(addBookmarkNewNeeds);
						...
						for (FamilyPlanReqrdActnsDto familyPlanReqrdActnsDto : familyPlanNeedsDto
							...
							if(Objects.nonNull(familyPlanReqrdActnsDto.getIndTffService())) {
								...
								tmplatRsrcLi.addAll(tffEvidenceFormGroupList);
							}
							...
							if (FormConstants.Y.equalsIgnoreCase(familyPlanReqrdActnsDto.getIndCourtOrdrd())) {
								...
								addBookmarkCourt = createBookmark(BookmarkConstants.COURT_ORDERED, NO);
							}
							...
							/* artf130730 : Label conditionally displayed if Domain is manually added and is confirmed need in Reassesed FSNA*/
							BookmarkDto addBookmarkConfirmFSNA = new BookmarkDto();
							if (FormConstants.Y.equalsIgnoreCase(familyPlanNeedsDto.getIndFsnaCnfrmdNeed())) {
								 addBookmarkConfirmFSNA = createBookmark(BookmarkConstants.CONFIRM_FSNA,
										"Need Confirmed In FSNA");
								 ParentStrQuesBookmarkList.add(addBookmarkConfirmFSNA);
							}
				...
				for (FamilyPlanNeedsDto familyPlanNeedsDto1 : familyPlanPartcpntDto.getFamilyPlanNeedsDtoList()) {
					FormDataGroupDto tmplatParentStrQuesFrmDataGrpDto = createFormDataGroup(
							FormGroupsConstants.TMPLAT_PARENT_NEED_QUES2, FormGroupsConstants.TMPLAT_PARENT_NEED);
					List<BookmarkDto> ParentStrQuesBookmarkList = new ArrayList<BookmarkDto>();
					if (FormConstants.N.equalsIgnoreCase(familyPlanNeedsDto1.getIndPriorNeeds())) {
						BookmarkDto addBookmarkNeedDomain = createBookmarkWithCodesTable(BookmarkConstants.NEED_DOMAIN,
								familyPlanNeedsDto1.getCdDomain(), CodesConstant.FSNADOMN);
						BookmarkDto addBookmarkRsnAddtn = createBookmarkWithCodesTable(
								BookmarkConstants.REASON_FOR_ADDITION, familyPlanNeedsDto1.getCdRsnForAdtn(),
								CodesConstant.CFPRFADD);
						BookmarkDto addBookmarkCurrAssmtNeeds = createBookmark(BookmarkConstants.CURR_ASSMT_NEED,
								formatTextValue(familyPlanNeedsDto1.getTxtCurrAssmntNeed()));
						BookmarkDto addBookmarkNewNeeds = createBookmark(BookmarkConstants.ASSMT_NEED,
								formatTextValue(familyPlanNeedsDto1.getTxtNeedDescription()));
						BookmarkDto addBookmarkEvalProgress = createBookmark(BookmarkConstants.PROGRESS_EVAL,
								formatTextValue(familyPlanNeedsDto1.getTxtEvalProgress()));
						...
						List<FormDataGroupDto> tmpParentReqNeedGrpLi = new ArrayList<FormDataGroupDto>();
						for (FamilyPlanReqrdActnsDto familyPlanReqrdActnsDto : familyPlanNeedsDto1
								.getFamilyPlanReqrdActnsDtoList()) {
							FormDataGroupDto tmplatReqActionFrmDataGrpDto = createFormDataGroup(
									FormGroupsConstants.TMPLAT_REQ_ACTION2,
									FormGroupsConstants.TMPLAT_PARENT_NEED_QUES2);
							List<BookmarkDto> reqActBookmarkLi = new ArrayList<BookmarkDto>();
							BookmarkDto addBookmarkRequiredAction = createBookmark(BookmarkConstants.REQUIRED_ACTION,
									formatTextValue(familyPlanReqrdActnsDto.getTxtReqrdActn()));
							BookmarkDto dtCmpltdBookMark = createBookmark(BookmarkConstants.DT_CMPLT,
							...
							/* artf130730 : Label conditionally displayed if Domain is manually added and is confirmed need in Reassesed FSNA*/
							BookmarkDto addBookmarkConfirmFSNA = new BookmarkDto(); 
							if (FormConstants.Y.equalsIgnoreCase(familyPlanNeedsDto1.getIndFsnaCnfrmdNeed()))  {
								 addBookmarkConfirmFSNA = createBookmark(BookmarkConstants.CONFIRM_FSNA,
										"Need Confirmed In FSNA");
								 ParentStrQuesBookmarkList.add(addBookmarkConfirmFSNA);
							}
							...
							for (FamilyPlanActnRsrcDto familyPlanActnRsrcDto : familyPlanReqrdActnsDto
								...
								FormDataGroupDto tmplatRrscFrmDataGrpDto = createFormDataGroup(
										FormGroupsConstants.TMPLAT_PARENT_RSRC2,
										FormGroupsConstants.TMPLAT_REQ_ACTION2);
								BookmarkDto addBookmarkNmRrsc = createBookmark(BookmarkConstants.NM_RESOURCE,
										familyPlanActnRsrcDto.getNmRsrc());
								BookmarkDto addBookmarkRsrcPhone = createBookmark(BookmarkConstants.RESOURCE_PHONE,
		...
		if (!familyPlanDtlEvalRes.getCdStage().equalsIgnoreCase("FSU")) {
			...
			for (FamilyPlanPartcpntDto familyPlanPartcpntDto : familyPlanDtlEvalRes.getFamilyPlanPartcpntDto()) {
				if (familyPlanPartcpntDto.getIndPartcpntType().equalsIgnoreCase("C")) {
					...
					for (FamilyPlanNeedsDto familyPlanNeedsDto : familyPlanPartcpntDto.getFamilyPlanNeedsDtoList()) {
						...
						if (FormConstants.Y.equalsIgnoreCase(familyPlanNeedsDto.getIndPriorNeeds())) {
							...
							BookmarkDto addBookmarkRsnAddtn = createBookmarkWithCodesTable(
									...
									CodesConstant.CFPRFADD);
							BookmarkDto addBookmarkCurrAssmtNeeds = createBookmark(BookmarkConstants.CURR_ASSMT_NEED_CHILD,
									formatTextValue(familyPlanNeedsDto.getTxtCurrAssmntNeed()));
							BookmarkDto addBookmarkNewNeeds = createBookmark(BookmarkConstants.ASSMT_NEED_CHILD,
									formatTextValue(familyPlanNeedsDto.getTxtNeedDescription()));
							BookmarkDto addBookmarkEvalProgress = createBookmark(BookmarkConstants.PROGRESS_EVAL_CHILD,
									formatTextValue(familyPlanNeedsDto.getTxtEvalProgress()));
							ChildStrQuesBookmarkList.add(addBookmarkCurrAssmtNeeds);
							ChildStrQuesBookmarkList.add(addBookmarkNeedDomain);
							ChildStrQuesBookmarkList.add(addBookmarkRsnAddtn);
							ChildStrQuesBookmarkList.add(addBookmarkEvalProgress);
							ChildStrQuesBookmarkList.add(addBookmarkNewNeeds);
							ChildStrQuesBookmarkList.add(addBookmarkNeedDomain);
							ChildStrQuesBookmarkList.add(addBookmarkRsnAddtn);
							List<FormDataGroupDto> tmpParentReqNeedGrpLi = new ArrayList<FormDataGroupDto>();
							for (FamilyPlanReqrdActnsDto familyPlanReqrdActnsDto : familyPlanNeedsDto
								...
								BookmarkDto addBookmarkRequiredAction = createBookmark(
										BookmarkConstants.REQUIRED_ACTION_CHILD,
										formatTextValue(familyPlanReqrdActnsDto.getTxtReqrdActn()));
								...
								for (FamilyPlanActnRsrcDto familyPlanActnRsrcDto : familyPlanReqrdActnsDto
									...
									BookmarkDto addBookmarkRsrcCity = createBookmark(
											BookmarkConstants.RESOURCE_CITY_CHILD,
											familyPlanActnRsrcDto.getAddrRsrcCity());

									BookmarkDto addBookmarkRsrcZip = createBookmark(
											BookmarkConstants.RESOURCE_ZIP_CHILD,
											familyPlanActnRsrcDto.getAddrRsrcZip());
									List<BookmarkDto> rsrcBookmarkLi = new ArrayList<BookmarkDto>();
									rsrcBookmarkLi.add(addBookmarkRsrcExt);
									rsrcBookmarkLi.add(addBookmarkRsrcPhone);
					...
					for (FamilyPlanNeedsDto familyPlanNeedsDto3 : familyPlanPartcpntDto.getFamilyPlanNeedsDtoList()) {
						FormDataGroupDto tmplatChildStrQuesFrmDataGrpDto = createFormDataGroup(
								FormGroupsConstants.TMPLAT_CHILD_NEED_QUES2, FormGroupsConstants.TMPLAT_CHILD_NEED);
						List<BookmarkDto> ChildStrQuesBookmarkList = new ArrayList<BookmarkDto>();
						if (FormConstants.N.equalsIgnoreCase(familyPlanNeedsDto3.getIndPriorNeeds())) {
							BookmarkDto addBookmarkNeedDomain = createBookmarkWithCodesTable(
									BookmarkConstants.NEED_DOMAIN_CHILD, familyPlanNeedsDto3.getCdDomain(),
									CodesConstant.FSNADOMN);
							BookmarkDto addBookmarkRsnAddtn = createBookmarkWithCodesTable(
									BookmarkConstants.REASON_FOR_ADDITION_CHILD, familyPlanNeedsDto3.getCdRsnForAdtn(),
									CodesConstant.CFPRFADD);
							BookmarkDto addBookmarkCurrAssmtNeeds = createBookmark(BookmarkConstants.CURR_ASSMT_NEED_CHILD,
									formatTextValue(familyPlanNeedsDto3.getTxtCurrAssmntNeed()));
							BookmarkDto addBookmarkNewNeeds = createBookmark(BookmarkConstants.ASSMT_NEED_CHILD,
									formatTextValue(familyPlanNeedsDto3.getTxtNeedDescription()));
							BookmarkDto addBookmarkEvalProgress = createBookmark(BookmarkConstants.PROGRESS_EVAL_CHILD,
									formatTextValue(familyPlanNeedsDto3.getTxtEvalProgress()));
							ChildStrQuesBookmarkList.add(addBookmarkCurrAssmtNeeds);
							ChildStrQuesBookmarkList.add(addBookmarkNeedDomain);
							ChildStrQuesBookmarkList.add(addBookmarkRsnAddtn);
							ChildStrQuesBookmarkList.add(addBookmarkEvalProgress);
							...
							ChildStrQuesBookmarkList.add(addBookmarkRsnAddtn);
							List<FormDataGroupDto> tmpParentReqNeedGrpLi = new ArrayList<FormDataGroupDto>();
							for (FamilyPlanReqrdActnsDto familyPlanReqrdActnsDto : familyPlanNeedsDto3
									.getFamilyPlanReqrdActnsDtoList()) {
								FormDataGroupDto tmplatReqActionFrmDataGrpDto = createFormDataGroup(
										FormGroupsConstants.TMPLAT_CHILD_REQ_ACTION2,
										FormGroupsConstants.TMPLAT_CHILD_NEED_QUES2);
								List<BookmarkDto> reqActBookmarkLi = new ArrayList<BookmarkDto>();
								BookmarkDto addBookmarkRequiredAction = createBookmark(
										BookmarkConstants.REQUIRED_ACTION_CHILD,
										formatTextValue(familyPlanReqrdActnsDto.getTxtReqrdActn()));
								...
								for (FamilyPlanActnRsrcDto familyPlanActnRsrcDto : familyPlanReqrdActnsDto
										.getFamilyPlanActnRsrcDtoList()) {
									FormDataGroupDto tmplatRrscFrmDataGrpDto = createFormDataGroup(
											FormGroupsConstants.TMPLAT_CHILD_RSRC2,
											FormGroupsConstants.TMPLAT_CHILD_REQ_ACTION2);
									BookmarkDto addBookmarkNmRrsc = createBookmark(BookmarkConstants.NM_RESOURCE_CHILD,
											familyPlanActnRsrcDto.getNmRsrc());
									BookmarkDto addBookmarkRsrcPhone = createBookmark(
		...
		if (!ObjectUtils.isEmpty(familyPlanDtlEvalRes.getFamilyPlanDtlEvalDto().getCdFgdmConference())
				&& !familyPlanDtlEvalRes.getFamilyPlanDtlEvalDto().getCdFgdmConference().equalsIgnoreCase("10")) {
			BookmarkDto bookmarkFgdm = createBookmarkWithCodesTable(BookmarkConstants.FGDM_HEADER,
					familyPlanDtlEvalRes.getFamilyPlanDtlEvalDto().getCdFgdmConference(), CodesConstant.CFGDMCON);
			bookmarkNonFrmGrpList.add(bookmarkFgdm);
		...
		BookmarkDto bookmarkCurrRvw = createBookmark(BookmarkConstants.DT_CURR_REVIEW,
				TypeConvUtil.formDateFormat(familyPlanDtlEvalRes.getFamilyPlanDtlEvalDto().getDtCurrentReview()));
		bookmarkNonFrmGrpList.add(bookmarkCurrRvw);
		...
		if (!ObjectUtils.isEmpty(familyPlanDtlEvalRes.getFamilyPlanDtlEvalDto().getTxtAdtnlCmntDngr())) {
			...
			BookmarkDto bookmark = createBookmark(BookmarkConstants.ADDITIONAL_INFO_3,
		...
		if (!ObjectUtils.isEmpty(familyPlanDtlEvalRes.getFamilyPlanDtlEvalDto().getTxtAdtnlCmntGoal())) {
			...
			BookmarkDto bookmark = createBookmark(BookmarkConstants.ADDITIONAL_INFO_4,
		...
		if (!ObjectUtils.isEmpty(familyPlanDtlEvalRes.getFamilyPlanDtlEvalDto().getTxtAdtnlFosCarePrevPlan())) {

			FormDataGroupDto template = createFormDataGroup(FormGroupsConstants.TMPLAT_ADDNL_FOSTER_CARE_PREV_PLAN,
					FormConstants.EMPTY_STRING);
			List<BookmarkDto> bookmarkList = new ArrayList<BookmarkDto>();

			BookmarkDto bookmark = createBookmark(BookmarkConstants.ADDNL_FOSTER_CARE_PREV_PLAN,
					formatTextValue(familyPlanDtlEvalRes.getFamilyPlanDtlEvalDto().getTxtAdtnlFosCarePrevPlan()));
			bookmarkList.add(bookmark);
			template.setBookmarkDtoList(bookmarkList);
			formDataGroupList.add(template);
		}
