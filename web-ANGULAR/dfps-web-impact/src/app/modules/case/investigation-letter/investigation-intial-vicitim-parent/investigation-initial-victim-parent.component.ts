...
import { InvestigationInitialVictimParentValidator } from './investigation-initial-victim-parent.validator';
...
@Component({
  selector: 'investigation-initial-vicitim-parent',
  templateUrl: './investigation-initial-victim-parent.component.html',
  styleUrls: ['./investigation-initial-victim-parent.component.css']
})
export class InvestigationInitialVictimParentComponent extends DfpsFormValidationDirective implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.navigationService.setTitle('Investigation Initial Letter to Parent of Victim - 2892');
    ...
    this.investigationLetterForm = this.formBuilder.group(
      {
        ...
        validators: [
          InvestigationInitialVictimParentValidator.requiredFieldValidations,
          InvestigationInitialVictimParentValidator.validateEmailPattern(),
          InvestigationInitialVictimParentValidator.phoneNumberPattern,
          InvestigationInitialVictimParentValidator.zipPattern
        ]
      }
    );
  }
  ...
  loadInvestigationLetterFormData() {
    this.letterTypeDecode = this.investigationLetterRes.letterTypes.find(value => 'IIL' === value.code).decode;
  ...
  delete() {
    if(this.investigationLetter.id){
      const initialState = {
        title: 'Investigation Initial Letter to Parent of Victim - 2892',
  ...
  downloadDraft() {
    if(this.isFormDirty()) {
      ...
      const letterName = this.investigationLetterRes.letterTypes.find(value => 'IIL' === value.code).decode;
  ...
  download() {
    let letterName = this.investigationLetterRes.letterTypes.find(value =>  'IIL' === value.code).decode;
  ...
  searchPerson() {
    ...
    const redirectUrl = 'case/letters/investigation-letters/IIL/' +
  ...
  updateIndex(isChecked: boolean, indexNumber: number){
    this.investigationLetter.initialLetterAllegations[indexNumber].checked = isChecked;
    this.store.dispatch(SET({ dirtyCheck: { isDirty: true } }));
  }
  ...
  getFormValue() {
    ...
    Object.keys(this.investigationLetter.initialLetterAllegations).forEach( key => {
      let val = this.investigationLetter.initialLetterAllegations[key].checked;
      formValue += key + '=[' + (val !== null ? val : '') + '],';
    });
    return formValue;
  }
